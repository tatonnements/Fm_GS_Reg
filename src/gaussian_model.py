...
from gaussian_model_utils import (RGB2SH, build_scaling_rotation,
                                            inverse_sigmoid,
                                            strip_symmetric)
...
class GaussianModel:
    def __init__(self, sh_degree: int = 3, isotropic=False):
        ...
        self.kf_id_to_gaussian_id = {}  # for every keyframe store its gaussian ids
    ...
    def restore_from_params(self, params_dict, training_args):
        ...
        self.kf_id_to_gaussian_id = params_dict.get("keyframe_id_to_gaussian_id", {})
    ...
    def capture_dict(self):
        return {
            ...
            "keyframe_id_to_gaussian_id": self.kf_id_to_gaussian_id
        }
    ...
    def add_points(self, pcd: o3d.geometry.PointCloud, frame_id: int = 0):
        self.kf_id_to_gaussian_id[frame_id] = self.get_size() + torch.arange(np.asarray(pcd.points).shape[0])
    ...
    def prune_points(self, mask):
        for kf_id, kf_gaussian_ids in self.kf_id_to_gaussian_id.items():
            kf_gaussian_ids = kf_gaussian_ids.to(mask.device)
            gaussian_ids_to_remove = torch.where(mask == 1)[0].detach()
            kf_gaussian_ids_to_remove = ~torch.isin(kf_gaussian_ids, gaussian_ids_to_remove)
            self.kf_id_to_gaussian_id[kf_id] = kf_gaussian_ids[kf_gaussian_ids_to_remove]
